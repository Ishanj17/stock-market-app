  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-gray-900">$6,855.00</p>
            </div>
            <div className="text-center">
                          ...
                          <div className="flex-shrink-0 h-10 w-10">
                            <div className="h-10 w-10 rounded-full bg-gradient-to-r from-teal-500 to-green-500 flex items-center justify-center">
                              <span className="text-white font-bold text-sm">
                                {investment.type === 'Stock' ? 'S' : investment.type === 'Mutual Fund' ? 'M' : 'I'}
                              </span>
                            </div>
                          </div>
                          <div className="ml-4">
                            {investment.type === 'Stock' ? (
                              <button 
                                onClick={() => navigate(`/stocks/detail/${investment.symbol}`)}
      ...
      {sellModalOpen && selectedInvestment && (
        <Modal isOpen={sellModalOpen} onClose={handleSellCancel}>
          <div className="bg-white rounded-lg p-6 max-w-md mx-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Sell Investment</h3>
              <button
              ...
              <>
                ...
                <div className="bg-gray-50 rounded-lg p-4 mb-6">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <div className="font-medium text-gray-900">{selectedInvestment.name}</div>
                    </div>
                    <div>
                      <span className="text-gray-500">Symbol:</span>
                      <div className="font-medium text-gray-900">{selectedInvestment.symbol}</div>
                    </div>
                    <div>
                      <span className="text-gray-500">Type:</span>
                      <div className="font-medium text-gray-900">{selectedInvestment.type}</div>
                    </div>
                    <div>
                      <span className="text-gray-500">Current Price:</span>
                      <div className="font-medium text-gray-900">₹{selectedInvestment.currentPrice.toFixed(2)}</div>
                    </div>
                    <div>
                      <span className="text-gray-500">Available Quantity:</span>
                      <div className="font-medium text-gray-900">{selectedInvestment.quantity}</div>
                    </div>
                    <div>
                      <span className="text-gray-500">Total Value:</span>
                      <div className="font-medium text-gray-900">₹{selectedInvestment.totalValue.toFixed(2)}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <input
                    ...
                    value={sellQuantity}
                    onChange={(e) => setSellQuantity(Math.min(parseInt(e.target.value) || 1, selectedInvestment.quantity))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    Max: {selectedInvestment.quantity} units
                  </p>
                </div>
                ...
                <div className="bg-blue-50 rounded-lg p-3 mb-6">
                  <div className="text-sm text-blue-800">
                    ...
                    <div className="text-lg font-bold text-blue-900">
                      ₹{(sellQuantity * selectedInvestment.currentPrice).toFixed(2)}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 px-4 py-2 bg-red-600 text-white rounded-md font-medium hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                  >
                    {sellLoading ? (
                      <>
